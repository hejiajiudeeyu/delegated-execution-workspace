...
当前 bundle 为 `changes/CHG-2026-101.yaml`。

## 当前判断

CHG-2026-101 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `deployability:overview` 已成为 Local Agent Loop、All-in-One Demo、Selfhost
  Platform、Public Stack、Operator Onboarding 和 Published Image 路径的只读第一张
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-101`
...
All-in-One Demo、Selfhost Platform、Public Stack 和 Management Console。页面明确区分
ready now 与 planned：当前本地 loop、all-in-one demo、selfhost、public-stack
safety checks、published-image smoke 和 Operator Onboarding 已可解释。Management
