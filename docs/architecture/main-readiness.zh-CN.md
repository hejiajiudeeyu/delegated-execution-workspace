...
当前 bundle 为 `changes/CHG-2026-047.yaml`。

## 当前判断

CHG-2026-047 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  security-review / audit-export / ops-report / backup-validate / restore-plan gate
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:ops-report` /
...
`selfhost:ops-report`、`selfhost:backup-validate`、`selfhost:restore-plan`，分别作为
公开暴露前安全、证据、交接报告、备份工件校验和恢复演练命令。仍未 ready 的能力
不被包装成绿色状态。页面重复强调 secrets、public origin、billing readiness 不能
被包装成绿色状态。
