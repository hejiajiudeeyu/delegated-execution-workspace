...
当前 bundle 为 `changes/CHG-2026-045.yaml`。

## 当前判断

CHG-2026-045 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  security-review / audit-export / backup-validate / restore-plan gate
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` /
...
public-stack 命令示例已加入 `selfhost:security-review`、`selfhost:audit-export` 和
`selfhost:backup-validate`、`selfhost:restore-plan`，分别作为公开暴露前安全、证据、
