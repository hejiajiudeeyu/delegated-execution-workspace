...
当前 bundle 为 `changes/CHG-2026-084.yaml`。

## 当前判断

CHG-2026-084 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-084`
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:backup-validate` / `selfhost:restore-plan` / `published-image:smoke -- --dry-run --json` 命令
