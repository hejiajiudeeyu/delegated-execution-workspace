...
当前 bundle 为 `changes/CHG-2026-052.yaml`。

## 当前判断

CHG-2026-052 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  doctor / summary / security-review / audit-export / ports / ops-report /
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-052`
...
- `selfhost:doctor` / `selfhost:init` / `selfhost:summary` / `selfhost:urls` /
  `selfhost:preflight`：已新增为 self-host 管理骨架，用于部署诊断、生成 env、
  输出一屏 profile 概要、发现 profile，并在 `up` 前检查 routes、compose config
  和 secret hygiene
...
- `test:selfhost-kit`：通过，用临时 profile 覆盖 env 生成、只读 doctor 输出、
  secret rotation dry-run / confirm 行为、一屏 summary 输出、public-stack preflight
  safety、非破坏性 security review、不泄漏 secret 的 admin audit export、restore
  planning 和 public route-contract smoke 输出
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:doctor` /
...
`selfhost:doctor`、`selfhost:summary`、`selfhost:ports`、`selfhost:ops-report`、`selfhost:backup-validate`、
