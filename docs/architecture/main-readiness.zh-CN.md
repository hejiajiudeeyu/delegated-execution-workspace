...
- `repos/brand-site`：`0336a81abbba30ad2b70fae836bc0e501f9a6b04`

当前 bundle 为 `changes/CHG-2026-054.yaml`。

## 当前判断

CHG-2026-054 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  profiles / quickstart / doctor / summary / security-review / audit-export / ports /
  ops-report / backup-validate / restore-plan gate
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-054`
...
- `selfhost:profiles` / `selfhost:quickstart` / `selfhost:doctor` / `selfhost:init` /
  `selfhost:summary` / `selfhost:urls` / `selfhost:preflight`：已新增为
  self-host 管理骨架，用于发现 profile、复制粘贴启动序列、部署诊断、生成 env、
  输出一屏 profile 概要、发现 URLs，并在 `up` 前检查 routes、compose config 和
  secret hygiene
...
- `test:selfhost-kit`：通过，用临时 profile 覆盖 env 生成、只读 profile map 输出、
  只读 doctor 输出、只读 quickstart 序列、secret rotation dry-run / confirm 行为、
  一屏 summary 输出、public-stack preflight safety、非破坏性 security review、
  不泄漏 secret 的 admin audit export、restore planning 和 public route-contract
  smoke 输出
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:profiles` / `selfhost:quickstart` / `selfhost:doctor` /
...
`selfhost:profiles`、`selfhost:quickstart`、`selfhost:doctor`、`selfhost:summary`、`selfhost:ports`、`selfhost:ops-report`、`selfhost:backup-validate`、
`selfhost:restore-plan`，分别作为公开暴露前安全、证据、端口可见性、交接报告、
启动序列、备份工件校验和恢复演练命令。仍未 ready 的能力不被包装成绿色状态。
