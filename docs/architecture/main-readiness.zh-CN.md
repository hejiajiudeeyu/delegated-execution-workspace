...
当前 bundle 为 `changes/CHG-2026-048.yaml`。

## 当前判断

CHG-2026-048 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-045`
...
`exports/selfhost/<profile>/`，记录 URLs、secret hygiene 状态和 operator 命令，
但不写入 raw secret 值；
