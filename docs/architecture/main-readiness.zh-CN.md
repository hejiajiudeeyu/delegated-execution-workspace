...
- `deployability:menu` 已成为给人和管理 UI 使用的只读第一屏 operator menu。它支持
  ...
  也支持 `--profile <key-or-alias>` 输出包含 selected runbook metadata 的聚焦菜单。
  它会展示 attention、primary command、runbook、action-plan、dashboard、handoff 和
  command catalog 入口，同时不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、
  不打印 secret 值
