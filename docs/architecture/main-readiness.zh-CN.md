...
当前 bundle 为 `changes/CHG-2026-055.yaml`。

## 当前判断

CHG-2026-055 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `selfhost:audit-export` 已成为本地 JSON 审计证据导出 helper，调用现有
  platform admin audit endpoint，但不打印 admin key
- `selfhost:ops-report` 已成为不含 secrets 的 Markdown 运维交接报告，记录
  URLs、host ports、secret hygiene 状态和后续命令
...
- `selfhost:readiness` 已成为只读部署就绪总览，合并 profile 文件、`.env` 状态、
  secret hygiene、public-stack origin / route 阻断项、URLs、声明 host ports 和下一步命令
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  ports / ops-report / backup-validate / restore-plan gate
...
- platform-first/operator-first onboarding 已有第四仓契约检查，能验证
  ...
  叙事、source fallback runbook、声明 host-port 盘点和不含 secrets 的 ops
  交接报告保持一致
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-055`
...
- `selfhost:profiles` / `selfhost:quickstart` / `selfhost:readiness` / `selfhost:doctor` / `selfhost:init` /
  `selfhost:summary` / `selfhost:urls` / `selfhost:preflight`：已新增为
  self-host 管理骨架，用于发现 profile、复制粘贴启动序列、只读 readiness 总览、
  部署诊断、生成 env、输出一屏 profile 概要、发现 URLs，并在 `up` 前检查 routes、
...
- `test:selfhost-kit`：通过，用临时 profile 覆盖 env 生成、只读 profile map 输出、
  只读 doctor 输出、只读 quickstart 序列、只读 readiness 总览、secret rotation dry-run /
  confirm 行为、一屏 summary 输出、public-stack preflight safety、非破坏性
...
- `operator:onboarding:check`：通过，验证 public-stack `/console/` 与
  ...
  runbook 叙事、`selfhost:ports`、`selfhost:ops-report` 一致
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:profiles` / `selfhost:quickstart` / `selfhost:readiness` / `selfhost:doctor` /
  `selfhost:summary` / `selfhost:ports` / `selfhost:ops-report` / `selfhost:backup-validate` /
  `selfhost:restore-plan` 命令
...
`corepack pnpm run selfhost:readiness` 会输出只读部署就绪总览，合并 profile 文件、
`.env` 状态、secret hygiene、public-stack origin / route 阻断项、URLs、声明 host
ports 和下一步命令，但不会调用 Docker、绑定 socket、探测网络、修改文件或打印
secret 值；
...
public-stack 命令示例已加入 `selfhost:security-review`、`selfhost:audit-export`、
`selfhost:profiles`、`selfhost:quickstart`、`selfhost:readiness`、`selfhost:doctor`、`selfhost:summary`、`selfhost:ports`、`selfhost:ops-report`、`selfhost:backup-validate`、
`selfhost:restore-plan`，分别作为公开暴露前安全、证据、端口可见性、交接报告、
启动序列、readiness 总览、备份工件校验和恢复演练命令。仍未 ready 的能力不被包装成绿色状态。
