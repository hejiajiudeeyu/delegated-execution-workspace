...
当前 bundle 为 `changes/CHG-2026-091.yaml`。

## 当前判断

CHG-2026-091 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-091`
...
- `compat:status -- --json`：通过，在本次 bundle 更新前报告 `CHG-2026-090`
  与当前 protocol/client/platform/brand-site gitlinks 匹配；同时把既有 dirty
  submodule worktree 作为 warnings 暴露，而不是误判为 ledger blockers
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:backup-validate` / `selfhost:restore-plan` / `selfhost:rotate -- --profile public-stack --json` / `selfhost:rotate -- --profile public-stack --confirm --json` / `deployability:handoff` / `deployability:handoff -- --json` / `operator:onboarding:check -- --json` / `published-image:smoke -- --dry-run --json` 命令
