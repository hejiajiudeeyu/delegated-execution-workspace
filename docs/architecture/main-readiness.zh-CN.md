...
当前 bundle 为 `changes/CHG-2026-105.yaml`。

## 当前判断

CHG-2026-105 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  `recovery_evidence` 管线、pipeline_summaries 和 profile-specific command variants 的 inherited safety posture
...
- `deployability:overview` 已成为 Local Agent Loop、All-in-One Demo、Selfhost
  Platform、Public Stack、Recovery & Evidence、Operator Onboarding 和 Published Image 路径的只读第一张
  命令地图，并支持 `--json`，同时不读取 `.env`、不调用 Docker、不绑定端口、
  不探测网络、不打印 secret 值
- `deployability:quickstart` 已成为 fresh checkout 的只读首次使用指南，会列出
  Daily Development、All-in-One Demo、Selfhost Platform、Public Stack 和
  Release Review tracks、有序命令和 JSON 入口，同时不读取 `.env`、不调用 Docker、
...
- `deployability:explain` 已成为只读 operator explainer，用来解释架构、
  truth-source 边界、profile 选择、public exposure gates、production hardening
  和第四仓验证顺序，并支持 `corepack pnpm run deployability:explain` 和
  `corepack pnpm --silent run deployability:explain -- --json`，同时不读取 `.env`、
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:production` 已成为只读 production hardening 边界视图，并支持
  `corepack pnpm run deployability:production` 和
  `corepack pnpm --silent run deployability:production -- --json`，同时不读取
  `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值；它把 daily
  deployability 与 public exposure / formal production readiness 分开，并展示
  billing、email、marketplace 和 formal release gates；同时输出
  `production_readiness_remediation_plan` /
  `formal_production_readiness_remediation`，让 dashboard 和 agent 能渲染
  public exposure、billing、email transport、marketplace、formal release 与
  management evidence 的有序步骤，而不把 daily deployability 当成 production
  readiness
- `deployability:readiness` 已成为独立 daily-deployable scorecard，给人、
  CI 和管理 UI 使用，并支持 `corepack pnpm run deployability:readiness` 和
  `corepack pnpm --silent run deployability:readiness -- --json`，同时不读取
  `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:roadmap` 已成为给管理 UI 和规划评审使用的只读 PRD milestone
  view，并支持 `corepack pnpm run deployability:roadmap` 和
  `corepack pnpm --silent run deployability:roadmap -- --json`，同时不读取 `.env`、
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:status` 已成为给管理第一屏使用的 compact read-only operator
  status，并支持 `corepack pnpm run deployability:status` 和
  `corepack pnpm --silent run deployability:status -- --json`，同时不读取 `.env`、
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值；它只聚合 readiness、
  roadmap 和 public-stack recipe metadata，不成为新的 runtime truth source
- `deployability:gates` 已成为 public exposure 和 production hardening 的
  compact read-only gate checklist，并支持 `corepack pnpm run deployability:gates`
  和 `corepack pnpm --silent run deployability:gates -- --json`，同时不读取 `.env`、
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值；它只把 roadmap 和
  command catalog metadata 投影成管理面 gate cards，不执行底层 gate 命令
- `deployability:exposure` 已成为非破坏性的 public-stack exposure blocker
  snapshot，并支持 `corepack pnpm run deployability:exposure` 和
  `corepack pnpm --silent run deployability:exposure -- --json`；它运行现有
  public-stack security review，只为 compose config 调用 Docker，不启动服务、
  不绑定端口，并把 public exposure findings 放在 `exposure_blockers`；当当前
  blocker 是 localhost `PUBLIC_SITE_ADDRESS` 时，它还会输出
  `operator_next_action`，包含 `.env` 目标、手动设置 public origin 的步骤和
  security-review 复验命令。它还会输出 `pre_exposure_remediation_plan`，把
  配置 public origin、复跑 security review、确认 route contract、运行 onboarding
  check、运行 published-image dry-run 和导出 evidence 排成 public-stack 有序检查清单
- `deployability:release` 已成为非破坏性的 release candidate gate，并支持
  `corepack pnpm run deployability:release -- --image-tag <candidate-tag>` 和
  `corepack pnpm --silent run deployability:release -- --image-tag <candidate-tag> --json`；
  它聚合 production hardening、public exposure、published-image plan 和 dry-run
  smoke evidence，但不发布镜像或包、不启动服务、不探测 endpoint、不打印 secret 值
- `deployability:operator-checklist` 已成为非破坏性的 public-stack operator
  checklist，并支持
  ...
  它把 menu、recipe、onboarding、release / exposure gate、backup-plan 和 handoff
  evidence 聚合成 checklist groups，但不启动服务、不探测 endpoint、不发布 artifacts、
  不打印 secret 值
...
- `deployability:dashboard` 已成为给 dashboard 和 CI 使用的只读聚合 payload，
  ...
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:next` 已成为给人、dashboard 和 agent 使用的只读单一下一步动作
  决策命令。当调用方不想渲染完整 menu 时，它会聚合 menu、action-plan 和 gates
  metadata，支持 `corepack pnpm run deployability:next`、
  `corepack pnpm --silent run deployability:next -- --json` 和
  `--profile <key-or-alias>`；在 public exposure 仍处于 gated 状态时默认推荐
  public-stack safety gate，并且不执行推荐命令、不读取 `.env`、不调用 Docker、
  不绑定端口、不探测网络、不打印 secret 值。它的 public-stack decision 还会通过
  `detail_command` 和 `detail_json_command` 指向 `deployability:exposure`，让管理面
  可以单独获取 blocker-specific `operator_next_action` 修复建议
- `deployability:recipe` 已成为单个 profile 的只读线性首次运行配方，并支持
  `corepack pnpm run deployability:recipe -- --profile public-stack` 和
  `corepack pnpm --silent run deployability:recipe -- --profile public-stack --json`，
  同时不执行命令、不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:action-plan` 已成为 dashboard 与完整命令目录之间的只读 operator
  下一步动作选择器，会把 dashboard 和 command catalog metadata 合并成 profile 级
  recommended commands、dashboard-safe commands、public-exposure gates、
  service-touching commands、safety notes、JSON 入口、profile `attention`
  metadata 和顶层 `recommended_profile_keys`，并支持
  `corepack pnpm run deployability:action-plan` 和
  `corepack pnpm --silent run deployability:action-plan -- --json`；operator 或
  dashboard 可以用
  `corepack pnpm --silent run deployability:action-plan -- --list-profiles --json`
  列出支持的 profile keys、aliases、pipeline keys 和 purposes，且不调用
  dashboard/catalog metadata；同一个 selector 也可从 `deployability:quickstart`、
  `deployability:safety` 和 `deployability:commands -- --track daily_dev` 发现；
  operator 也可以用
  `corepack pnpm --silent run deployability:action-plan -- --profile public-stack --json`
  聚焦同一 schema，输出 `profile_filter`，并把未知 profile 名称作为 blockers 返回，
  同时不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值；
  管理面可以用 `attention.rank` 排序 cards，并在公网暴露前突出
  `attention.level=safety_gate`
...
- `deployability:commands` 已成为给人、dashboard 和 CI 使用的只读命令目录，
  ...
  首次使用 track 和 pipeline 过滤的列表，并通过 `filters.profiles` 输出 profile
  selector metadata；`--profile <key-or-alias>` 是 pipeline filter 的
  operator-friendly alias 层；带 profile 参数的命令变体继承基础安全姿态，并支持
  `corepack pnpm run deployability:commands`、
  `corepack pnpm --silent run deployability:commands -- --json` 和
  `corepack pnpm --silent run deployability:commands -- --profile public-stack --json`，
  同时不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:runbook` 已成为给 operator 和管理 UI 使用的只读 profile runbook
  投影。它支持 `corepack pnpm run deployability:runbook` 和
  `corepack pnpm --silent run deployability:runbook -- --json` 输出 profile index，
  也支持 `--profile <key-or-alias>` 输出阶段化 `profile_runbook`，按 inspect、gate、
  start、verify、operate、evidence 组织命令。该投影复用 profile catalog 与 command
  catalog，让 gate 位于 start 之前，并且不读取 `.env`、不调用 Docker、不绑定端口、
  不探测网络、不打印 secret 值
- `deployability:menu` 已成为给人和管理 UI 使用的只读第一屏 operator menu。它支持
  `corepack pnpm run deployability:menu` 和
  `corepack pnpm --silent run deployability:menu -- --json` 输出全部 profile choices，
  也支持 `corepack pnpm run deployability:menu -- --profile public-stack` 和
  `corepack pnpm --silent run deployability:menu -- --profile public-stack --json`
  输出包含 selected runbook metadata 的聚焦菜单。
  对 public-stack 而言，聚焦 JSON 还会包含来自只读 `operator:onboarding:plan`
  投影的 `selected_onboarding_plan`，让管理 UI 可以在同一个第一屏展示
  `/console/` onboarding flow、gateway credential setup、smoke/evidence steps 和
  onboarding check。它还会输出 `operator_status_summary`、`operator_status_cards`
  和 `operator_next_decision`。状态投影基于 profiles、commands 和 console
  metadata 这些 menu-local 输入，并显式避免递归调用完整 `deployability:status`
  CLI；下一步决策投影使用当前选中的 profile 和共享 operator decision helper，并通过
  `source_status.operator_next_decision.avoids_recursive_next_cli=true` 标记它不会调用
  `deployability:next`。public-stack decision 会保留与 `deployability:next` 相同的
  `detail_command` 和 `detail_json_command` exposure-remediation 入口。它会展示
  attention、primary command、runbook、action-plan、dashboard、handoff、command
  catalog 和 operator onboarding 入口，同时不读取 `.env`、不调用 Docker、不绑定端口、
  不探测网络、不打印 secret 值
- ready-now 命令目录条目已具备明确 category / posture metadata，不再回退到
  `unmapped`；本地 `runtime_diagnostic`、本地 `runtime_acceptance` 和
  published-image `delegated_smoke` 路径都可被 dashboard 和 operator 过滤
...
- `deployability:handoff` 已成为不含 secrets 的生态级交接报告，会写入
  ...
  顶层 `profile_selector`、shared per-pipeline summaries、安全说明和下一步验证命令，
  并支持 `--json`，同时不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、
  不打印 secret 值
- `deployability:evidence` 已成为不含 secrets 的 evidence bundle 命令，供 operator
  和管理 UI 使用。它支持 `corepack pnpm run deployability:evidence -- --profile public-stack`
  和 `corepack pnpm --silent run deployability:evidence -- --profile public-stack --json`，
  会写出 manifest、聚焦 dashboard/menu/recipe/handoff/command-catalog JSON 和 handoff
  Markdown，不读取 `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
- `deployability:dashboard -- --json` 和 `deployability:handoff -- --json` 已把
  命令目录 profile 目录提升为顶层 `profile_selector`，管理面可以从任一聚合 payload
  渲染 profile 选择器，而不需要解析内部 section 路径
- `deployability:dashboard -- --profile public-stack --json` 和
  `deployability:handoff -- --profile public-stack --json` 已复用同一个 profile
  resolver，输出 `profile_filter`、聚焦命令目录 metadata 和单个所属 pipeline
  summary，同时让 ecosystem_readiness 继续表示全局 daily-deployable scorecard
- `deployability:dashboard -- --json` 和 `deployability:handoff -- --json` 也会输出
  `profile_summaries`，把 profile aliases / purpose 与 pipeline status、counts、
  next commands、safety notes、共享 `attention` metadata 和顶层
  `recommended_profile_keys` 合并成管理 UI card 可直接消费的数组
- `deployability:profiles` 已成为专用只读 profile-card catalog，供 operator、
  dashboard、CI 和管理脚本使用。它从 overview, command, and doctor metadata、
  共享 pipeline/profile summary helpers 和共享第四仓 profile registry 派生 cards，
  输出 labels、aliases、所属 pipeline keys、status、
  counts、next commands、next JSON commands、safety notes、共享 `attention`
  metadata 和顶层 `recommended_profile_keys`，并支持
  `corepack pnpm run deployability:profiles`、
  `corepack pnpm --silent run deployability:profiles -- --json` 和聚焦
  `--profile <key-or-alias>` payload，同时不读取 `.env`、不调用 Docker、
  不绑定端口、不探测网络、不打印 secret 值
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-105`
...
- `deployability:action-plan -- --json`：通过，输出 profile 级 recommended commands、
  dashboard-safe commands、public-exposure gates、service-touching commands、
  safety notes、next JSON commands、profile `attention` metadata 和
  `recommended_profile_keys`，不混入终端文本或 secret 值
- `deployability:action-plan -- --list-profiles --json`：通过，输出支持的
  profile keys、aliases、pipeline keys、purposes 和下一步 profile 命令，且不调用
  dashboard/catalog metadata、不泄漏 secret 值
- `deployability:quickstart`、`deployability:safety` 和
  `deployability:commands -- --track daily_dev`：通过，把 action-plan profile
  selector 作为 first-use、read-only、dashboard-safe 顶层命令暴露出来
- `deployability:action-plan -- --profile public-stack --json`：通过，只输出
  `public_stack`、`profile_filter` 和 public-stack 公开暴露 gate 命令；未知 profile
  名称会作为 blockers 返回，不泄漏 secret 值
...
- `deployability:commands -- --json`：通过，输出命令目录及 category、posture、track
  ...
  profile 参数的命令变体继承基础安全姿态；同时输出 `filters.profiles` selector
  metadata，不混入终端文本或 secret 值
- `deployability:dashboard -- --json` / `deployability:handoff -- --json`：通过，
  把同一份 profile selector 目录提升为顶层 `profile_selector`，供 dashboard 和
  handoff consumer 直接渲染
- `deployability:dashboard -- --profile public-stack --json` /
  `deployability:handoff -- --profile public-stack --json`：通过，把 `public-stack`
  解析为 `public_stack`，将 pipeline summaries 过滤到 public-stack 管线，并保留全局
  ecosystem_readiness
- `deployability:dashboard -- --json` / `deployability:handoff -- --json`：通过，
  输出作为派生管理卡片投影的 `profile_summaries`，全量模式覆盖所有 profiles，
  并包含共享 profile `attention` metadata 和顶层 `recommended_profile_keys`；
  focused 模式只返回一个 profile summary 和一个 `recommended_profile_keys`
- `deployability:commands -- --profile public-stack --json`：通过，把 operator profile
  alias 解析为 `public_stack`，只返回 public-stack pipeline 命令，并把未知 profile
  名称作为 blockers 返回，不混入终端文本或 secret 值
- `deployability:commands -- --json`：通过，ready-now 命令路径不再出现
  `unmapped` category / posture
...
第四仓的 `deployability:console -- --json` 现在会消费 client-owned
`corepack pnpm --dir repos/client run check:ops-console-runtime-surface`
evidence command；当这些 Runtime wiring 存在时，`runtime_status` 会标记为
`client_owned_evidence_available`。
它也会消费
`corepack pnpm --dir repos/client run check:ops-console-settings-surface`；当
Preferences 和 Access Lists 证明审批模式、whitelist/Blocklist 字段和 `allow_all`
安全文案来自 client-owned policy surface 时，`settings_approval_policy` 会标记为
`client_owned_evidence_available`。
它也会消费
`corepack pnpm --dir repos/client run check:ops-console-logs-surface`；当 Runtime
和 Help 证明 `/runtime/logs`、`/runtime/alerts`、caller/responder/relay 日志服务、
日志筛选 metadata、`selfhost:logs` 和 secret-safety copy 来自 client-owned console
surface 时，`logs_guidance` 会标记为 `client_owned_evidence_available`。
...
All-in-One Demo、Selfhost Platform、Recovery & Evidence、Public Stack 和 Management Console。页面明确区分
ready now 与 planned：当前本地 loop、all-in-one demo、selfhost、public-stack
safety checks、recovery evidence、published-image smoke 和 Operator Onboarding 已可解释。Management
Console 文案也开始说明 admin-only Billing 页面是 operator surface，不等于
client-facing billing ready；public-stack 命令示例已加入
`selfhost:quickstart -- --profile all-in-one`、`selfhost:up -- --profile all-in-one`、
`all_in_one_demo`、`selfhost:security-review`、`selfhost:audit-export`、`selfhost:audit-export -- --json`、
`selfhost:profiles`、`selfhost:quickstart`、`selfhost:readiness -- --all`、`selfhost:readiness`、`selfhost:readiness -- --json`、`selfhost:doctor`、`selfhost:init -- --json`、`selfhost:init -- --profile public-stack --json`、`selfhost:summary`、`selfhost:ports`、`selfhost:up -- --json`、`selfhost:smoke -- --json`、`selfhost:logs -- --json`、`selfhost:down -- --json`、`selfhost:ops-report`、`deployability:safety`、`deployability:safety -- --json`、`deployability:doctor`、`corepack pnpm run deployability:doctor`、`corepack pnpm --silent run deployability:doctor -- --json`、`deployability:dashboard`、`corepack pnpm run deployability:dashboard`、`corepack pnpm --silent run deployability:dashboard -- --json`、`deployability:action-plan`、`corepack pnpm run deployability:action-plan`、`corepack pnpm --silent run deployability:action-plan -- --json`、`deployability:commands`、`corepack pnpm run deployability:commands`、`corepack pnpm --silent run deployability:commands -- --json`, `corepack pnpm run deployability:runbook`, `corepack pnpm --silent run deployability:runbook -- --json`、`deployability:commands -- --pipeline recovery_evidence`、`recovery_evidence`、`deployability:handoff`、`operator:onboarding:check -- --json`、`selfhost:backup-validate`、
...
启动序列、人工和机器可读 readiness 总览、选定 profile 部署地图、机器可读部署地图、安全启动/日志/停止命令 metadata、operator 下一步动作选择、机器可读交接、备份计划、备份工件校验、恢复演练、轮换计划和轮换 metadata 命令。仍未 ready 的能力不被包装成绿色状态。
