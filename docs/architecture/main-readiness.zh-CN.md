...
当前 bundle 为 `changes/CHG-2026-090.yaml`。

## 当前判断

CHG-2026-090 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-090`
...
- `compat:status -- --json`：通过，在本次 bundle 更新前报告 `CHG-2026-089`
