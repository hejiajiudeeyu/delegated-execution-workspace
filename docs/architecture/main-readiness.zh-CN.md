...
- `deployability:production` 已成为只读 production hardening 边界视图，并支持
  ...
  billing、email、marketplace 和 formal release gates
...
- `deployability:gates` 已成为 public exposure 和 production hardening 的
  ...
  不调用 Docker、不绑定端口、不探测网络、不打印 secret 值；它只把 roadmap、
  command catalog 和 status metadata 投影成管理面 gate cards，不执行底层 gate 命令
- `deployability:exposure` 已成为非破坏性的 public-stack exposure blocker
  ...
  不绑定端口，并把 public exposure findings 放在 `exposure_blockers`
...
- `deployability:menu` 已成为给人和管理 UI 使用的只读第一屏 operator menu。它支持
  ...
  onboarding check。它会展示 attention、primary command、runbook、action-plan、
  dashboard、handoff、command catalog 和 operator onboarding 入口，同时不读取
  `.env`、不调用 Docker、不绑定端口、不探测网络、不打印 secret 值
