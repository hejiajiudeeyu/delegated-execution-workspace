...
当前 bundle 为 `changes/CHG-2026-057.yaml`。

## 当前判断

CHG-2026-057 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  ports / ops-report / backup-validate / restore-plan / rotate-plan gate
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-057`
...
- `selfhost:profiles` / `selfhost:quickstart` / `selfhost:readiness -- --all` /
  ...
  单 profile 总览、部署诊断、生成 env、输出一屏 profile 概要、发现 URLs，并在 `up`
...
`selfhost:profiles`、`selfhost:quickstart`、`selfhost:readiness -- --all`、`selfhost:readiness`、`selfhost:readiness -- --json`、`selfhost:doctor`、`selfhost:summary`、`selfhost:ports`、`selfhost:ops-report`、`selfhost:backup-validate`、
`selfhost:backup-plan`、`selfhost:restore-plan`、`selfhost:rotate-plan`，分别作为公开暴露前安全、证据、端口可见性、交接报告、
启动序列、人工和机器可读 readiness 总览、备份计划、备份工件校验、恢复演练和轮换计划命令。仍未 ready 的能力不被包装成绿色状态。
