...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  security-review / audit-export / restore-plan gate
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:restore-plan`
  命令
...
`selfhost:restore-plan`，分别作为公开暴露前安全、证据和恢复演练命令。仍未 ready
的能力不被包装成绿色状态。页面重复强调 secrets、public origin、billing readiness
不能被包装成绿色状态。
