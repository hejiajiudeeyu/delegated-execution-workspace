...
当前 bundle 为 `changes/CHG-2026-050.yaml`。

## 当前判断

CHG-2026-050 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  security-review / audit-export / ports / ops-report / backup-validate / restore-plan gate
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-050`
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` / `selfhost:ports` /
  `selfhost:ops-report` / `selfhost:backup-validate` / `selfhost:restore-plan` 命令
