...
当前 bundle 为 `changes/CHG-2026-044.yaml`。

## 当前判断

CHG-2026-044 收口后，当前固定组合已经可以用于日常第四仓开发：
...
- brand-site 已有中英双语 Deployability Profiles 文档页，解释部署 profiles、
  ...
  security-review / audit-export gate
...
- `check:boundaries`：通过，已把 `@delexec/billing-store` 归入
  `platform/data`
- `check:bundles`：通过，使用 `CHG-2026-044`
...
- `test:selfhost-kit`：通过，用临时 profile 覆盖 env 生成、secret rotation
  ...
  、不泄漏 secret 的 admin audit export 和 public route-contract smoke 输出
...
- `repos/brand-site` `npm run smoke:deployability-content`：通过，覆盖中英双语
  ...
  `selfhost:security-review` / `selfhost:audit-export` 命令
...
public-stack 命令示例已加入 `selfhost:security-review` 和 `selfhost:audit-export`
作为公开暴露前安全和证据命令。仍未 ready 的能力不被包装成绿色状态。页面重复强调
