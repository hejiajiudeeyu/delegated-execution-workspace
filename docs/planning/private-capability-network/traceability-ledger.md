...
| FR-025 | 任务排队 (P1)（M3 单元 6） | protocol + platform + client | partial（超时预算已落，排队状态未落） | 协议侧 `queued`/`executing` 已区分且转移表齐全（protocol `d2ad83b`），**但全协议测试树对 QUEUED 零覆盖**（2026-08-10 grep 实证）；平台未实现，投影表 `EXECUTION_FROM_LEGACY_EVENT` 没有任何事件映射到 queued，而设备侧 workerConcurrency 默认 1、ack 时任务其实正在本地排队却被投影成 executing。**随行的真实问题**：三个独立 300s 时钟（caller `hard_timeout_s`、平台 `TOKEN_TTL_SECONDS` 兼资金 hold 过期、responder 热线 hard 180s）互不知情，2026-08-09 真实 MinerU（含模型加载约 4 分钟）第一次调用即被杀；派发信封根本不带 constraints，responder 侧 `task.constraints` 恒 null，`RESPONDER_MAX_HARD_TIMEOUT_S` 护栏在远程路径上形同虚设。**2026-08-10 超时半边已落地（CHG-2026-226，生产 v0.4.15）**：①协议侧 queued/executing 转移表补上 7 例测试（此前全树零覆盖，表本身是对的，属补证不属修错）；②`heldBillingExpiresAt` 不再用 `TOKEN_TTL_SECONDS`——「信封可以无人认领多久」与「真活可以跑多久」是两个问题，共用一个数会在任务**仍在执行时**退款并合成一条 FAILED；现独立为 `EXECUTION_BUDGET_S`（默认 1800s）且从**责任方最后一次出声**起算，因为还在报进度的责任方不是消失了的责任方；③派发信封开始携带 constraints，此前 `task.constraints` 恒 null 使两处已建成的机制在唯一要紧的路径上是死的；④由此浮出一个此前问不出来的问题——**这是谁的机器**：`resolveAdapterTimeouts` 取调用方与设备两者中**较小**的那个，调用方可以提前放弃（那是它自己的事），但不能替设备运营者决定这台机器跑多久，该函数已导出并有反向断言；⑤两侧默认值 60s/180s → 300s/1800s，够不到的上限是同一个 bug 的另一面。**仍缺**：`queued` 生命周期事件与投影（ACKED 仍被投影成 executing，而设备侧其实在本地排队）、capacity 入流 |
| FR-040 | 自动交付校验 | protocol + platform + client | **done**（代码侧 + 生产已滚） | M3 单元 1，CHG-2026-220。此前 `schema_valid` 是执行方写在自己结果对象里的字段，平台除评审测试外**从不读它**，却据此结钱——拿钱的一方是唯一判定活干没干完的一方。**开工时才确认的结构性事实**：正常路径上平台**从未收到结果包**（responder 签名结果经 relay 直达 caller，平台只收裸终态事件），全仓无 result 路由，所以「平台复核」是**传输改动**而非加一次校验调用。现落地：`validateDeliveredOutput`（contracts 0.1.10）按 Call **钉住的**那份 `output_schema` 判，两侧各跑一次——responder 签名前自校（失败还来得及如实描述），平台事后按冻结契约独立复核（「责任方说它检查过」正是本条要替换的断言）；签名结果随 COMPLETED 事件上行，带尺寸上限，超限判 unchecked 而非拒收（为信封大小丢掉已完成的工作，CHG-2026-216 犯过一次）。结果签名扩入 `hotline_version`——此前 v1 的结果与 v2 的在密码学上无法区分，而本条恰恰要按钉住的那份判；**追加式**：结果里没有该 key，canonical 形式里就没有，旧责任方签的字节逐字节不变，有钉死字节的断言。**四个等级而非两个**：verified / failed / **unchecked**（契约没声明 output_schema、或结果不够判）/ **unverified**（无 pin、无签名、或大到送不过来）——把后两者报成 verified 就是把同一个自我断言往里挪一层，而单元 2 的验收钟就要从「verified」起表。轴还区分了两种空白：尚未交付 vs 完成于本检查存在之前（永不补判）。**刻意不设闸**：判 failed 的交付照旧结算，只是被记录了，有测试按名断言这一现状。**发布顺序不可倒且已遵守**：canonical 表只在「旧签名方/新校验方」方向兼容，故 contracts 先发（run 31328518840）→ 校验方（平台 v0.4.13 + caller）→ 签名方（ops 0.1.14）。测试：协议 165 单测、client 162 单测 + 22 集成（5 新）、平台 59 单测 + 203 集成（9 新），新行为断言均经 stash 证明改动前红。**生产已滚 v0.4.13**（认证组合 `v0.4.13-ops.0.1.15`），`release-manifest check https://callanything.xyz` 报 `runtime matches release v0.4.13-ops.0.1.15`。**批末追加 CHG-2026-222**：CI（不是本地）抓到一条既有缺陷——`finalizeTask` 里紧邻着两个同类 best-effort 调用，lifecycle 事件被 try/catch 保护而指标上报没有；由于 `failTask` 自己就调 `finalizeTask`，指标上报在处理失败时抛出，会在 catch 出口再抛一次并逃逸成未处理拒绝，**把原始错误一并带走**。表现是「137 例全过、套件仍然失败」。已修并发 ops 0.1.15。**仍缺**：结算等验收（单元 2）；生产上一次真实 MinerU 调用走完新链路的实证（设备需升 ops 0.1.14） |
| FR-041 | Caller 接受触发结算 | platform + client | **done**（代码侧） | M3 单元 2，CHG-2026-223 + 224（**生产已滚 v0.4.14**，`runtime matches release v0.4.14`）。此前责任方发一条 COMPLETED 就同时结束了工作与释放了钱——**拿钱的一方决定自己什么时候拿到钱**。现结算与执行拆开：FAILED 照旧立即退款（没有人该为了拿回自己的钱去"接受"一次失败），COMPLETED 只开验收窗；能结算的只有两条路——`POST /v1/requests/:id/accept`（caller 鉴权 + 所有权）与窗口到期。既有的 `billing.state === "held"` 闸原地不动但承重变大：它是"显式接受与到期扫描相撞时只结一次"的结构保证，有专门断言。客户端侧 caller-skill-adapter 增 `accept_delivery`——只有拿运营者 shell 才能做的接受不算调用方拥有接受权；这也是该适配器**第一次以某个身份**调平台（此前所有平台读取都打在免鉴权的公开目录上）。**无需协议发版**（验收轴与转移表 M1 已冻结，平台事件名是本地词汇），contracts 仍为 0.1.10 |
| FR-042 | 一次修订 | platform + client | **done**（代码侧） | M3 单元 3，CHG-2026-225。A-06「一次范围内修订不额外计费、原 hold 保持」**由"不产生第二笔 hold"自然落地**，而不是靠某处的一条规则：钱根本没动，所以没有"不收"这回事。重交付**重开**验收窗而非再加一个。**两处不改就会让修订静默失效的既有缺陷一并修**：①终态事件去重按 (event_type, responder, hotline) 匹配，修订后的第二个 COMPLETED 会被当重复**吞掉**——收不到判定、进不了时间线、也到不了结算；现改为 attempt 级（progress 事件早已这么解过），无 attempt_id 的事件仍只与无 attempt_id 的匹配，存量调用不受影响。②responder 侧 inbox 只按 request_id 去重并**重放存好的签名结果**，修订会原样退回调用方刚刚反对的那份输出、只换个时间戳（此为开工前报 owner 并获批的范围增量）。重执行的尝试**另立 task 记录**，被修订的那次保留自己的签名结果——journal 与"第一次到底交付了什么"都依赖它存活 |
| FR-043 | 修订范围约束 | platform | **done**（代码侧） | M3 单元 3。第二次修订直接拒绝（`REVISION_ALREADY_USED`）：越界或追加的工作是**另起一次 Call**，再给一次免费轮次等于对固定价买到的范围做一次无声的无偿扩张。计数落在 Call 上而非从事件日志推断——「这单修订过几次」必须活过事件历史修剪。范围之尺 = Call 钉住的 `hotline_version`（随响应返回），永远不是目录现值 |
| FR-044 | 验收窗口与自动接受 | platform | **done**（代码侧） | M3 单元 2，CHG-2026-223 + 224（生产已滚 v0.4.14）。A-05：按档位 quick 24h / standard 72h / deep 7d，网络边界 24h–7d，显式 `acceptance_window_s` 优先。**时钟起点是 verified delivery 不是 COMPLETED**——后者是拿钱那一方的自述，拿它起表等于让责任方决定调用方的异议权何时开始。窗口长度只读 Call 在签发 token 时快照的那份 terms，绝不读目录现值：发布者改自己的验收窗，不得缩短一个已经在跑的窗口（有断言）。自动接受沿用本服务既有的时间形态——读取时懒评估，与 hold 过期并排；本服务没有调度器，per-call 定时器也活不过重启。**本单元自行拍的一个决定（已上报 owner）**：平台判为 `failed` 的交付**到期不自动接受**。沉默可以是对"满足了契约的工作"说"行"的方式，不可以是对"平台自己认定没满足契约的工作"付钱的方式；这类调用保持冻结并浮到运营者面前，而运营者自 v0.4.9 起就有 close+退款的路子，所以在单元 3 的争议流程到位之前也不存在无处可去的钱。**顺带必改**：`funds_held_after_end` 注意项——已交付调用上的冻结资金过去意味着"没人注意到的钱"，现在是最长七天的**正常状态**；不改的话每一个健康的待验收调用都会每个重播周期骚扰运营者一次，而为正确行为而响的告警正是运营者学会无视告警feed的方式 |
| FR-045 | 争议 | platform | **done**（代码侧） | M3 单元 3。理由必填（与本系统其他每一处裁定同一门槛）。settlement 转 `blocked`——**堵出口而不搬钱**：钱在 hold 时就已离开余额，争议冻结的是 settle 与本会走到它的自动接受；有断言证明验收窗到期不会跨过一个未决争议。顺带修掉：`buildCallStateAxes` 此前把任何未识别的 billing 状态静默映成 `none`，于是一笔争议中的调用会被报成"根本没有计费"——正是那种让冻结的结算彻底隐形的读法 |
| FR-046 | Operator 契约判断 | platform | todo | 只判履约，不裁真理 |
| FR-050 | 预算预扣 | platform | **done** | hold 已实现；`rejected` 不产生 hold 待 FR-021 |
| FR-051 | 成功结算 | platform | **done**（代码侧） | 验收门已落（CHG-2026-223）：settle 只有显式接受与到期自动接受两个调用方，责任方的任何上报都不再触发结算 |
| FR-052 | 失败退款 | platform | **done** | FAILED 自动退款 + 过期 hold 惰性回收（T-503 实证） |
| FR-053 | 争议冻结 | platform | **done**（代码侧） | M3 单元 3，随 FR-045 落地：`billing.state = "blocked"`，出口仍走既有 exactly-once 闸 |
