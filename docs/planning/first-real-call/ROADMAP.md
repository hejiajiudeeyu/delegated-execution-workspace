...
Status: Wave 3 T-302 caller billing read API locally verified

Scope for this slice:

- T-302 platform-owned caller self-service billing read API.
- Add `GET /v1/tenants/me/balance` and `GET /v1/tenants/me/ledger` using the authenticated caller API key.
- Keep tenant ownership implicit via `auth.user_id`; the caller surface does not accept arbitrary tenant ids.
- Return a caller-friendly `ERR_BILLING_NOT_ENABLED` response when a caller has no billing tenant yet.

Why this slice first:

- T-302 removes the "blind debit" gap after T-301: callers can inspect their balance and ledger before UI work.
- The billing RFC freezes general `/v1/tenants/{tenant_id}` resources, but this first caller-facing slice intentionally exposes only `/me` routes to avoid over-read risk.
- T-303 can now build a balance/ledger console route against a real platform API instead of admin-only billing routes.
...
| T-303 billing console route | `repos/platform` | next | Depends on T-302 API shape; now unblocked. |
| T-304 paid-call e2e | fourth repo | blocked | Depends on T-303 or enough caller-facing balance UX to exercise a paid call end to end. |
