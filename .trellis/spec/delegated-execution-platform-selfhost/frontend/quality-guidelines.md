...
delegated-execution-platform-selfhost is part of platform repository umbrella. Quality means preserving the owning repository boundary, keeping tests near the behavior, and proving cross-repo compatibility when contracts or submodule SHAs move.
...
- Run `npm --prefix repos/platform test` when this area changes.
- Run `npm --prefix repos/platform run test:service:packages` when this area changes.
- Run `npm --prefix repos/platform run test:deploy:config` when this area changes.
