...
delegated-execution-client is part of client repository umbrella. Quality means preserving the owning repository boundary, keeping tests near the behavior, and proving cross-repo compatibility when contracts or submodule SHAs move.
...
- Run `npm --prefix repos/client test` when this area changes.
- Run `npm --prefix repos/client run test:packages` when this area changes.
...
- Use repos/client/apps/ops/src/cli.js as a current reference.
- Use repos/client/apps/ops-console/src/App.tsx as a current reference.
- Use repos/client/packages/runtime-utils/src/index.js as a current reference.
- Use repos/client/tests/unit/ops-console.catalog.test.tsx as a current reference.
